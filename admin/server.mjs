...
import { startInstagramSync, syncReels, verifyToken as verifyIgToken, getState as igState, disable as igDisable, fetchReelByUrl } from './instagram-sync.mjs';
...
createServer(async (req, res) => {
  ...
  try {
    ...
    if (p.startsWith('/api/admin/')) {
      ...
      /* ---- Instagram reels auto-sync (see admin/instagram-sync.mjs) ----
         Status is visible to both roles; setting/removing the TOKEN is owner-only:
         it is a credential for the owner's Instagram account. */
      if (p === '/api/admin/instagram' && req.method === 'GET') {
        const s = igState();
        return send(res, 200, { enabled: !!s.token, username: s.username || '', lastSync: s.lastSync || '', lastError: s.lastError || '', lastCount: s.lastCount || 0 });
      }
      if (p === '/api/admin/instagram' && req.method === 'POST') {
        if (role !== 'owner') return send(res, 403, { error: 'Only the owner can set the Instagram token' });
        const b = await readJsonBody(req);
        const token = b && String(b.token || '').trim();
        if (!token) return send(res, 400, { error: 'Paste the access token' });
        try {
          const me = await verifyIgToken(token);
          syncReels(rebuild).catch(() => {}); // first sync in the background
          return send(res, 200, { ok: true, username: me.username || '' });
        } catch (err) {
          return send(res, 400, { error: `Instagram rejected the token: ${err.message}` });
        }
      }
      if (p === '/api/admin/instagram/sync' && req.method === 'POST') {
        try { return send(res, 200, await syncReels(rebuild)); }
        catch (err) { return send(res, 400, { error: err.message }); }
      }
      if (p === '/api/admin/instagram' && req.method === 'DELETE') {
        if (role !== 'owner') return send(res, 403, { error: 'Only the owner can disable the sync' });
        igDisable();
        return send(res, 200, { ok: true });
      }

      /* Resolve one Instagram reel link to self-hosted media, for Admin → Story
         Reels. Content operation, not a credential one (the token is used but
         never returned), so both roles may call it — same rule as /sync. */
      if (p === '/api/admin/storyreel/fetch' && req.method === 'POST') {
        const b = await readJsonBody(req);
        if (!b || !b.url) return send(res, 400, { error: 'Paste the Instagram reel link first.' });
        try { return send(res, 200, await fetchReelByUrl(b.url)); }
        catch (err) { return send(res, 400, { error: err.message }); }
      }
