...
| **Search widget** | Control the home "Search any disease" widget. Add/remove **topics**; per topic set the **label** (Popular-chip + result badge text), **keywords** (comma-separated match terms), a **Popular chip** toggle (which chips show under the search box), the **Related-articles** blogs (tick from your posts), the **Doctor** (Auto = nephrologist, a specific doctor, or RIIMS Care Team) and the **Video/reel** (Auto = first reel, or a specific one). Saved to `search.topics`; drives `site/js/search-data.js` + the Popular chips (see §8). | **Auto-doctor rule** (topic's Doctor left on "Auto"): a `/nephro/` match in title/specialties wins first; else the roster doctor whose name equals `SPECIALIST_DOCTOR.name` (Dr. Abhishek Gupta — the site's canonical kidney physician); else kidney-ish specialties; else the first doctor. The old rule matched only `/nephro/`, so when the mislabelled "Senior Nephrologist" title was corrected the search silently fell back to the first roster doctor (it showed Dr. Vikas for "Kidney" — fixed 2026-07-22).
